...
public class TopicAspect {
    ...
    @AfterReturning(value = "execution(* com.masterjava.news.services.TopicService.*(..))")
    public void afterReturningAdvice(JoinPoint joinPoint)
    {
        Operation op = Helpers.getOperation(joinPoint.getSignature().toString());
        auditRepository.addRecord(new Audit(op, ObjectType.Topic, joinPoint.getSignature().toString(), new Date()));
