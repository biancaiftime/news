...
public class CommentAspect {
    ...
    @AfterReturning(value = "execution(* com.masterjava.news.services.CommentService.*(..))")
    public void afterReturningAdvice(JoinPoint joinPoint)
    {
        Operation op = Helpers.getOperation(joinPoint.getSignature().toString());
        auditRepository.addRecord(new Audit(op, ObjectType.Comment, joinPoint.getSignature().toString(), new Date()));
